...
const Points = () => {
    ...
    const [itemsSetected, setItemsSetected] = useState<number[]>([]);
    ...
    function handleItemsSelected(id: number) {
        const alreadySelected = itemsSetected.findIndex(item => item === id);

        if (alreadySelected >= 0) {
            const filteredItems = itemsSetected.filter(item => item !== id);
            setItemsSetected(filteredItems);
        } else {
            setItemsSetected([...itemsSetected, id]);
        }
    }
                ...
                >
                    ...
                    {items.map(item => (
                        ...
                        <TouchableOpacity
                            ...
                            style={[
                                styles.item,
                                itemsSetected.includes(item.id) ? styles.selectedItem : {}
                            ]}
                            onPress={() => handleItemsSelected(item.id)}
                            activeOpacity={0.6}
                        >
